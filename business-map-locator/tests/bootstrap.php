...
if (!function_exists('delete_term_meta')) { function delete_term_meta(int $id, string $key): bool { $GLOBALS['bml_test_delete_term_meta_calls']=(int)($GLOBALS['bml_test_delete_term_meta_calls']??0)+1; if (!isset($GLOBALS['bml_test_term_meta'][$id][$key])) return false; unset($GLOBALS['bml_test_term_meta'][$id][$key]); return true; } }
if (!function_exists('wp_delete_term')) { function wp_delete_term(int $id, string $taxonomy): bool|WP_Error { $GLOBALS['bml_test_wp_delete_term_calls']=(int)($GLOBALS['bml_test_wp_delete_term_calls']??0)+1; if (!isset($GLOBALS['bml_test_terms'][$taxonomy][$id])) return false; unset($GLOBALS['bml_test_terms'][$taxonomy][$id], $GLOBALS['bml_test_term_meta'][$id]); return true; } }
if (!function_exists('wp_remove_object_terms')) { function wp_remove_object_terms(int $id, array $terms, string $taxonomy): array|WP_Error { $GLOBALS['bml_test_wp_remove_object_terms_calls']=(int)($GLOBALS['bml_test_wp_remove_object_terms_calls']??0)+1; $current=array_map('intval',$GLOBALS['bml_test_post_terms'][$id][$taxonomy]??[]);$remove=array_map('intval',$terms);$GLOBALS['bml_test_post_terms'][$id][$taxonomy]=array_values(array_diff($current,$remove));return $remove; } }
