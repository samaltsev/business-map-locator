            ...
            <main class="bml-editor-main">
                ...
                <section class="bml-editor-section is-open" data-section-card="classification">
                    ...
                    <div id="bml-section-classification-body" class="bml-section-body"><div class="bml-form-grid">
                        ...
                        <?php endforeach; ?>
                        <div class="bml-term-field"><label class="bml-field"><span><?php esc_html_e('Area', 'business-map-locator'); ?></span><select name="area_id"><option value="0"><?php esc_html_e('No Area assigned', 'business-map-locator'); ?></option><?php foreach ($areas as $term) : $leaf=true; foreach ($areas as $candidate) if ((int)$candidate->parent===(int)$term->term_id) $leaf=false; $inactive=(string)get_term_meta($term->term_id,'bml_area_active',true)==='0'; $current=(string)($area_ids[0]??'')===(string)$term->term_id; ?><option value="<?php echo esc_attr((string)$term->term_id); ?>" <?php selected($current); disabled(!$leaf || ($inactive && !$current)); ?>><?php echo esc_html(str_repeat('— ', (int)$term->parent ? 1 : 0) . $term->name . (!$leaf ? ' (' . __('parent', 'business-map-locator') . ')' : '')); ?></option><?php endforeach; ?></select><small><?php esc_html_e('Only active leaf Areas can be newly assigned.', 'business-map-locator'); ?></small></label></div>
                    </div></div>
                </section>
                            ...
                            <div class="bml-hours-editor__days">
                                <?php foreach (['Mon' => __('Monday', 'business-map-locator'), 'Tue' => __('Tuesday', 'business-map-locator'), 'Wed' => __('Wednesday', 'business-map-locator'), 'Thu' => __('Thursday', 'business-map-locator'), 'Fri' => __('Friday', 'business-map-locator'), 'Sat' => __('Saturday', 'business-map-locator'), 'Sun' => __('Sunday', 'business-map-locator')] as $day_code => $day_label) : ?>
                                    <div class="bml-hours-day" data-hours-day="<?php echo esc_attr($day_code); ?>" data-hours-label="<?php echo esc_attr($day_label); ?>">
                                        ...
                                        <span>–</span>
                                        <input type="time" data-hours-close disabled aria-label="<?php echo esc_attr(sprintf(__('Closing time on %s', 'business-map-locator'), $day_label)); ?>">
                                    </div>
                                <?php endforeach; ?>
                            </div>
