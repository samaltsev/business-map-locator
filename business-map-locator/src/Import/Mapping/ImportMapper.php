...
final class ImportMapper {
    ...
    public function map(array $headers,array $row): array {$data=array_combine($headers,$row);if(!is_array($data)){return [];}if((string)apply_filters('bml_import_unknown_columns_policy','reject',[])==='ignore'){$data=array_intersect_key($data,array_flip(self::ALLOWED_COLUMNS));}if(isset($data['status'])){$data['status']=$this->status($data['status']);}if(isset($data['operational_status'])){$data['operational_status']=$this->operationalStatus($data['operational_status']);}if(isset($data['visible'])){$data['visible']=$this->boolean($data['visible']);}return $data;}
    public function validate(array $data): array {$title=sanitize_text_field((string)($data['title']??$data['name']??''));$lat=$this->coordinate($data['lat']??null,-90,90);$lng=$this->coordinate($data['lng']??null,-180,180);if($title===''||$lat===null||$lng===null){return ['valid'=>false,'error'=>'Title or coordinates are invalid. Coordinates must use a dot as decimal separator.'];}if(array_key_exists('operational_status',$data)&&$data['operational_status']===null){return ['valid'=>false,'error'=>'Operational status must be active, temporarily_closed, hidden, or open.'];}return ['valid'=>true,'title'=>$title,'lat'=>$lat,'lng'=>$lng];}
    ...
    private function boolean(mixed $value): string {$v=strtolower(trim((string)$value));if($v===''){return '';}return in_array($v,['1','true','yes','y','on','visible','publish'],true)?'1':'0';}
    private function status(mixed $value): string {$v=strtolower(trim((string)$value));if($v===''){return '';}return in_array($v,['draft','0','false','no','inactive','disabled'],true)?'draft':'publish';}
    private function operationalStatus(mixed $value): ?string {$value=trim((string)$value);if($value===''){return '';} $value=sanitize_key($value);if($value==='open'){return 'active';}return in_array($value,['active','temporarily_closed','hidden'],true)?$value:null;}
