...
final class AreaMigrationExecutor
{
    ...
    /** @param array<string,mixed> $snapshot */ private function hasBlockers(array $snapshot): bool { $plan=(array)($snapshot['plan']??[]);return !empty($plan['planning_blockers'])||!empty($plan['collision_list'])||!empty($plan['ambiguous_list'])||!empty($plan['decision_required_list']); }
