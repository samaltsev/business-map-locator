...
final class AreaMigrationExecutor
{
    ...
    /** @param array<string,mixed> $decision @param array<string,mixed> $snapshot */ private function executeCity(string $runId,array $decision,array $snapshot): void { $cityId=(int)$decision['city_id'];$plan=(array)$decision['planned_area'];$city=$this->term($cityId,'bml_city');if($city===null)throw new \RuntimeException('CITY_MISSING');$snapCity=null;foreach((array)($snapshot['terms']['bml_city']??[]) as $candidate)if((int)($candidate['id']??0)===$cityId)$snapCity=$candidate;if($snapCity===null||(string)$snapCity['name']!==$city['name']||(string)$snapCity['slug']!==$city['slug'])throw new \RuntimeException('CITY_DRIFT');$identity=['city_term_id'=>$cityId,'planned_area'=>$plan];$create=$this->journal->planOperation($runId,2,'CREATE_AREA',$identity);$area=$this->findArea($plan);if($area===null){$create=$this->start($runId,$create);$created=wp_insert_term((string)$plan['name'],'bml_area',['slug'=>(string)$plan['slug'],'parent'=>0]);if(is_wp_error($created))throw new \RuntimeException('AREA_CREATE_FAILED');$area=$this->term((int)$created['term_id'],'bml_area');if($area===null)throw new \RuntimeException('AREA_CREATE_UNVERIFIABLE');$this->checkpoint('AFTER_AREA_CREATED');$create=$this->journal->markApplied($runId,$create['operation_key'],['city_term_id'=>$cityId,'area_term_id'=>$area['id'],'area_name'=>$area['name'],'area_slug'=>$area['slug'],'parent'=>$area['parent']]);}else{if($area['name']!==(string)$plan['name']||$area['slug']!==(string)$plan['slug']||$area['parent']!==0)throw new \RuntimeException('AREA_COLLISION');if(($create['state']??'')===AreaMigrationJournal::PLANNED)throw new \RuntimeException('AREA_COLLISION');if(($create['state']??'')===AreaMigrationJournal::STARTED)$create=$this->journal->markApplied($runId,$create['operation_key'],['city_term_id'=>$cityId,'area_term_id'=>$area['id'],'area_name'=>$area['name'],'area_slug'=>$area['slug'],'parent'=>0]);}$create=$this->finish($runId,$create);$this->writeProvenance($runId,'WRITE_CITY_PROVENANCE',['city_term_id'=>$cityId,'area_term_id'=>$area['id']],$cityId,'_bml_area_term_id',$area['id']);$this->writeProvenance($runId,'WRITE_AREA_PROVENANCE',['area_term_id'=>$area['id'],'city_term_id'=>$cityId],$area['id'],'_bml_migrated_from_city_term_id',$cityId);if((int)get_term_meta($cityId,'_bml_area_term_id',true)!==$area['id']||(int)get_term_meta($area['id'],'_bml_migrated_from_city_term_id',true)!==$cityId)throw new \RuntimeException('RECIPROCAL_VERIFICATION_FAILED'); }
    /** @param array<string,mixed> $identity */ private function writeProvenance(string $runId,string $type,array $identity,int $termId,string $key,int $expected): void { $op=$this->journal->planOperation($runId,2,$type,$identity);$current=(int)get_term_meta($termId,$key,true);if($current!==0&&$current!==$expected)throw new \RuntimeException('PROVENANCE_CONFLICT');if(($op['state']??'')===AreaMigrationJournal::PLANNED)$op=$this->start($runId,$op);if($current!==$expected){update_term_meta($termId,$key,$expected);$this->checkpoint($type==='WRITE_CITY_PROVENANCE'?'AFTER_CITY_PROVENANCE_WRITTEN':'AFTER_AREA_PROVENANCE_WRITTEN');$op=$this->journal->markApplied($runId,$op['operation_key'],['term_id'=>$termId,'previous_value'=>$current,'written_value'=>$expected]);}elseif(($op['state']??'')===AreaMigrationJournal::STARTED)$op=$this->journal->markApplied($runId,$op['operation_key'],['term_id'=>$termId,'previous_value'=>$current,'written_value'=>$expected]);$this->finish($runId,$op); }
